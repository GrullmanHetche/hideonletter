...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-black text-stone-400 font-sans selection:bg-stone-800 selection:text-white flex flex-col">
      {/* 상단 네비게이션 */}
      <nav className="p-6 md:p-10 flex justify-between items-center w-full">
        <span className="text-xs tracking-[0.4em] font-black text-white uppercase">HIDEONLETTER</span>
        <div className="flex space-x-6 text-[10px] tracking-widest text-stone-600 uppercase font-medium">
      ...
      {/* 메인 컨텐츠 영역 */}
      <div className="flex-1 flex flex-col justify-center px-8 md:px-20 py-20 max-w-4xl">
        ...
        <div className="max-w-md space-y-8 text-[14px] leading-relaxed font-light text-stone-500 break-keep">
          ...
          <div className="pt-6 flex flex-col space-y-6">
            ...
            {/* 💌 시크릿 편지 버튼을 여기로 옮겼어요! (찾기 쉽게) */}
            <div 
              onClick={() => setShowSecret(true)}
              className="group cursor-pointer inline-flex items-center space-x-4 pt-4"
            >
              <span className="text-lg">💌</span>
              <div className="flex flex-col">
                <span className="text-[10px] tracking-[0.4em] font-bold uppercase text-stone-800 group-hover:text-white transition-all">Private Message</span>
                <span className="text-[9px] text-stone-700 font-light tracking-widest group-hover:text-stone-400">내 속마음이 알고 싶어?</span>
              </div>
            </div>
          </div>
        </div>
            ...
            {!isUnlocked ? (
              <div className="flex flex-col items-center space-y-8 py-20">
                ...
                <input 
                  ...
                  autoFocus
                />
                <p className="text-[9px] text-stone-900 tracking-widest uppercase">Hint: Your Birthday</p>
              </div>
            ) : (
              <div className="space-y-8 py-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
                ...
                <div className="text-[15px] leading-loose text-stone-300 font-light break-keep">
                  "오늘 경기 너무 멋있었어요! 😍 중계 화면으로 보니까 조그매서 뭔가 귀여웠어요. <br /><br />
                  과제도 많고 할 공부도 많은 하루지만 2세트에서 깔끔하게 끝내서 너무 힘났당 ₍ᐢɞ̴̶̷.̮ɞ̴̶̷ᐢ₎ "
                </div>
                <div className="text-right pt-8 uppercase">
                  <p className="text-[10px] text-stone-600 tracking-[0.3em] italic">- Logged by Yu-Hyun -</p>
                </div>
              </div>
            )}
          </div>
      ...
      {/* 푸터 */}
      <footer className="p-6 md:p-10 flex flex-col md:flex-row justify-between items-start md:items-end space-y-4 md:space-y-0 text-[10px] tracking-widest text-stone-700 uppercase w-full">
        ...
        <div className="flex space-x-12">
          <div className="space-y-1">
            ...
            <p className="text-[11px] text-stone-500 tracking-widest font-medium">S.H.Lee & Y.H.Lee</p>
          </div>
        </div>
      </footer>
